...
class Gotrip extends CI_Controller {
    ...
    public function login_submit(){
        ...
        if($remember) {
            ...
            set_cookie("username", $logindata['username'], '300');
            set_cookie("password", $logindata['password'], '300');
        ...
        if(!is_null($response)){ // check if username & password are correct
            $sess_data = array(
                'username' => $response['username'],
                'password' => $response['password'],
                'logged_in' => TRUE
  ...
  //function to check if user is in session
    ...
    public function forgot_submit() {
        ...
        $response = $this->my_model->check_forgot_password($logindata);
        if (!is_null($response)) {
            $check = $this->my_model->change_pass($logindata['password']);

            if (!is_null($check)) {
                $validation['success'] = true;
                $validation['message'] = "Password successfully recovered!";
    ...
    public function user_profile($username){
        ...
        if($username != $this->session->userdata('username')){    
            $config['base_url'] = base_url().'gotrip/profile';
    ...
    public function post_submit(){
        ...
        if(!$this->upload->do_upload('gotrip-img')){
            $data = array(
    ...
    public function search(){
        ...
        $config['base_url'] = base_url().'gotrip/search';
    ...
    public function comment($post_id){
        ...
        if($data == true){
            ...
            }elseif(strpos($url, 'gotrip')){
                $post = str_replace('/gotrip','',$url);
                redirect('gotrip'.$post);
